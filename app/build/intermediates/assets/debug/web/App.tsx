...
import { 
  ...
  Download, CheckCircle, Shield, UserX, BellRing
} from 'lucide-react';
...
// 2. Podcast List
const PodcastList: React.FC<{ onOpen: (id: string) => void }> = ({ onOpen }) => {
  ...
  return (
    <div className="p-4 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="space-y-4">
        {filtered.map(pod => (
          <div key={pod.id} onClick={() => onOpen(pod.id)} className="group bg-zinc-900 rounded-xl overflow-hidden border border-zinc-800 cursor-pointer active:scale-[0.98] transition-all">
            ...
            <div className="p-4">
                <div className="flex justify-between items-start mb-1">
                    <h3 className="font-semibold text-lg line-clamp-1">{pod.title}</h3>
                ...
                <div className="flex items-center justify-between text-zinc-500 text-xs">
                    <div className="flex items-center space-x-3">
                        <span className="flex items-center"><Heart size={14} className="mr-1"/> {pod.likesCount}</span>
                        <span className="flex items-center"><MessageSquare size={14} className="mr-1"/> {pod.commentsCount}</span>
                    </div>
...
// 3. Podcast Detail
const PodcastDetail: React.FC<{ id: string, user: User, onBack: () => void, onRefreshUser: (u: User) => void }> = ({ id, user, onBack, onRefreshUser }) => {
  ...
  const handleLike = () => {
    setIsLiked(!isLiked);
    DB.toggleLike(podcast.id, !isLiked);
    setPodcast(prev => prev ? ({...prev, likesCount: prev.likesCount + (isLiked ? -1 : 1)}) : undefined);
  };

  const handleSave = () => {
    setIsSaved(!isSaved);
    DB.toggleSave(user.id, podcast.id);
  };
  
  const handleDownload = () => {
      setIsDownloaded(!isDownloaded);
      DB.toggleDownload(user.id, podcast.id);
      // Refresh local user state for accurate button status on re-entry
  ...
  return (
    <div className="animate-in slide-in-from-bottom-10 duration-300 bg-zinc-950 min-h-full pb-20">
      ...
      <div className="sticky top-0 z-30 bg-black">
        ...
        />
        <button onClick={onBack} className="absolute top-4 left-4 bg-black/50 p-2 rounded-full backdrop-blur-md">
            <X size={20} />
        </button>
      </div>
...
const AddPodcast: React.FC<{ user: User, onComplete: () => void }> = ({ user, onComplete }) => {
  ...
  const [videoUrl, setVideoUrl] = useState('http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4');
  ...
  return (
    <div className="p-6 animate-in fade-in zoom-in duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1">Video Source (Mock URL)</label>
                <input type="text" value={videoUrl} onChange={e => setVideoUrl(e.target.value)}
                    className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-zinc-500 text-sm focus:outline-none" />
                <p className="text-[10px] text-zinc-600 mt-1">Using mock storage bucket for demo.</p>
            </div>
...
const Notifications: React.FC = () => {
    ...
    return (
        <div className="p-6">
            ...
            <div className="space-y-4">
                {notifications.map(n => (
                    <div key={n.id} className={`p-4 rounded-xl border ${n.read ? 'bg-zinc-900 border-zinc-800' : 'bg-indigo-900/10 border-indigo-500/30'}`}>
                        <div className="flex items-start space-x-3">
                            <div className={`mt-1 w-2 h-2 rounded-full ${n.read ? 'bg-transparent' : 'bg-indigo-500'}`}></div>
                            <div>
...
const Profile: React.FC<{ user: User }> = ({ user }) => {
  const [saved, setSaved] = useState<Podcast[]>([]);

  useEffect(() => {
    const all = DB.getPodcasts();
    setSaved(all.filter(p => user.savedPodcastIds.includes(p.id)));
  }, [user]);

  return (
    <div className="p-6">
        <div className="flex flex-col items-center mb-8">
            <img src={user.photoUrl} alt="Avatar" className="w-24 h-24 rounded-full border-4 border-zinc-800 mb-4" />
            <h2 className="text-2xl font-bold">{user.name}</h2>
            <p className="text-indigo-400 text-sm">{user.nickname}</p>
            <p className="text-zinc-500 text-sm mt-2 text-center max-w-xs">{user.description}</p>
        </div>

        <div className="bg-zinc-900 rounded-xl p-1 mb-6">
            <div className="grid grid-cols-2 text-center divide-x divide-zinc-800">
                <div className="p-3">
                    <span className="block text-xl font-bold text-white">{saved.length}</span>
                    <span className="text-xs text-zinc-500 uppercase tracking-wider">Saved</span>
                </div>
                <div className="p-3">
        ...
        <h3 className="font-bold mb-4 flex items-center"><Bookmark size={18} className="mr-2 text-indigo-500"/> Saved Podcasts</h3>
        <div className="space-y-3">
            {saved.length === 0 && <p className="text-zinc-600 text-sm">No saved podcasts yet.</p>}
            {saved.map(p => (
                <div key={p.id} className="flex items-center space-x-4 bg-zinc-900 p-3 rounded-lg border border-zinc-800">
...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    switch (view.type) {
      case 'PODCAST_LIST':
        return <PodcastList onOpen={(id) => setView({ type: 'PODCAST_DETAIL', podcastId: id })} />;
      case 'PODCAST_DETAIL':
        return <PodcastDetail id={view.podcastId} user={user} onBack={() => setView({ type: 'PODCAST_LIST' })} onRefreshUser={refreshUser}/>;
      case 'ADD_PODCAST':
        return <AddPodcast user={user} onComplete={() => setView({ type: 'PODCAST_LIST' })} />;
      case 'DOWNLOADS':
      ...
      case 'ADMIN_PANEL':
        // Basic protection: only render if role is admin
